...
- (void)beginSession:(CDVInvokedUrlCommand*)command {
    ...
    // PPM Patch
    BOOL invalidateAfterFirstRead = [command argumentAtIndex:0];

    NSLog(@"invalidateAfterFirstRead %d", invalidateAfterFirstRead);
...
-(void) fireSessionInvalidatedEvent:(NSInteger)reason {
    ...
    NSLog(@"fireSessionInvalidatedEvent: %@", reason);
